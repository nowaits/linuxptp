...
static void send_subscription(struct pmc_agent *node)
{
	...
	pmc_send_set_action(node->pmc, MID_C_SUBSCRIBE_EVENTS_NP, &sen, sizeof(sen));
}
...
static int renew_subscription(struct pmc_agent *node, int timeout)
{
	...
	res = run_pmc(node, timeout, MID_C_SUBSCRIBE_EVENTS_NP, &msg);
...
int run_pmc_wait_sync(struct pmc_agent *node, int timeout)
{
	...
	while (1) {
		res = run_pmc(node, timeout, MID_P_PORT_DATA_SET, &msg);
		if (res <= 0)
			return res;
...
int pmc_agent_query_dds(struct pmc_agent *node, int timeout)
{
	...
	res = run_pmc(node, timeout, MID_C_DEFAULT_DATA_SET, &msg);
	if (is_run_pmc_error(res)) {
		return run_pmc_err2errno(res);
	}
				    ...
				    int *tstamping, int *phc_index, char *iface)
{
	...
	while (1) {
		res = run_pmc(node, timeout, MID_P_PORT_PROPERTIES_NP, &msg);
		if (is_run_pmc_error(res)) {
			goto out;
		}
	...
	while (1) {
		res = run_pmc(node, timeout, MID_P_PORT_HWCLOCK_NP, &msg);
		if (is_run_pmc_error(res)) {
			goto out;
		}
...
int pmc_agent_query_utc_offset(struct pmc_agent *node, int timeout)
{
	...
	res = run_pmc(node, timeout, MID_C_TIME_PROPERTIES_DATA_SET, &msg);
	if (is_run_pmc_error(res)) {
		return run_pmc_err2errno(res);
	}
